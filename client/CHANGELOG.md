...
### Added
- **Window Activity Chain Tracking**: TUI capture worker now tracks window switches during each capture interval
  - Async background polling every 1 second to detect window changes
  - Last 5 window switches included in context for AI analysis
  - Window activity logs stored in database for stitching user sessions
  - Matches service daemon functionality for consistent context enrichment
  - Enables deeper understanding of user workflow across applications

## [0.1.8] - 2026-01-11

### Fixed
- **Portkey Logging**: Fixed missing context metadata in TUI capture worker
  - Window title and app name now properly logged to Portkey instead of showing "unknown"
  - Added WindowMonitor initialization to gather active window context
  - Context metadata (window_title, app_name, keystrokes, mouse_clicks) now passed to all analyze calls
  - Improves observability and debugging of AI analysis requests
